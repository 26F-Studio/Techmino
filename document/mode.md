...
|                    |                   |                                                              |
| **`sequence`**     | `bag`             | 序列模式，是放一块后对 `next` 序列的刷新函数，可以使用默认的几个函数用字符串表示，也可以自己写一个。注意：使用协程技术 |
| **`seqData`**      | `{1,2,3,4,5,6,7}` | 序列模式使用的“包”数据 （本质是生成序列用的数据，会作为参数传进上面那个叫 `sequence` 的序列生成函数，不一定是包） |
