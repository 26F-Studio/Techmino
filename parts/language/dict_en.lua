...
FNNS and     {"Support 3",
    ...
    {"King of Stackers",
        "kos kingofstackers",
        "game",
        "Browser | Multiplayer | Mobile Support\nKoS for short. A turn-based browser Tetris game. Players take turns placing blocks on their squares in 7-block cycles. Attacks only enter the field when a block is placed without clearing a row. The game is highly strategic and has several options for attack mechanics.",
        "https://kingofstackers.com/games.php",
    },
    {"Tetr.js",
        "tetrjs tetr.js",
        "game",
        "Browser | Singleplayer | Mobile Support\nA browser-based Tetris game with many advanced options and modes.\nLink to Farter's Dig Mod (which itself is a mod of another Tetris fan game). You can also check another mode on Tetris Wiki called \"Tetr.js enhanced.\"\n\n[MrZ: The visuals are simple with barely any animations. Only a few virtual key combinations are available for mobile devices.]",
        "http://farter.cn/t",
    },
    {"Tetra Legends",
        "tl tetralegends",
    ...
    {"Nullpomino",
        "np nullpomino",
        "game",
        "Windows/macOS/Linux | Singleplayer/Multiplayer\nNP for short. A highly-customizable professional Tetris game. Nearly every parameter in the game can be adjusted.\n\n[MrZ: It's UI style is like, retro. Also all controls are with keyboard, and new players may have some problems getting used to it. Also, it seems that macOS Monterey (or later) cannot run this game.]",
    },
    {"Misamino",
        "misamino",
        "game",
    ...
    {"Triple",
        "triple 3",
        "term",
        "Clear three lines at a time.",
    },
    {"Techrash",
        "techrash tetris 4",
        "term",
        "*Techmino-Exclusive*\nClear four lines at a time.",
    },
    {"Tetris",
        "tetris 4",
        "term",
    ...
    {"HPC",
        ...
        "term",
        "*Techmino-Exclusive*\nHalf Perfect Clear\nAn extension of All Clear/Perfect Clear. A clear is considered an HPC if it clears all minos including and above the line where the clear is performed. If only one line remains on the board after an HPC, that line must not contain any minos placed by the player. An HPC sends a small attack/defense.",
    },
    {"Spin",
        "spin",
        "term",
    ...
    {"TSS",
        "t1 tspinsingle",
        "term",
        "T-Spin Single\nClearinh one line with a T-spin.",
    },
    {"TSD",
        "t2 tspindouble",
        "term",
        "T-Spin Double\nClearinh two lines with a T-spin.",
    },
    {"TST",
        "t3 tspintriple",
        "term",
        "T-Spin Triple\nClearinh three lines with a T-spin.",
    },
    {"MTSS",
    ...
    {"Rotation Systems",
        ...
        "term",
        "Systems that determine how blocks rotate. In most modern Tetris games, the blocks rotate around a specific center of rotation (but some games may not have a fixed center of rotation). If any of the minoes overlap with either the wall of blocks on the board, the system will try to move the blocks in certain directions to avoid overlapping. This is known as \"wall-kicking.\". A rotation may fail if the offset is too large. Wall kicking allows minoes to move into certain shaped holes that would otherwise be unreachable. Different rotation systems may have different offset tables and orders.",
    },
    {"Orientation",
        "direction 0r2l 02 20 rl lr",
        "term",
        "In SRS or SRS-like rotation systems, there is a widely-accepted (though non-standard) notation for describing the orientations of the minoes:\n\nThe original orientation of the block (the orientation of the block when the block is spawned) is denoted as \"0\";\nThe orientation of the block after a 90° clockwise (right) rotation is denoted as \"R\";\nThe orientation of the block after a 90° counterclockwise (left) rotation is denoted as \"L\";\nThe orientation of the block after a 180°  rotation is denoted as \"2\".\n\nUnder this system, 0→L means rotating counterclockwise from the original orientation (0) to L; 0→R means rotating clockwise from the original orientation (0) to R; 2→R means rotating counterclockwise from 2 (180°) to R.",
    },
    {"ARS",
        "arikrotationsystem atarirotationsystem",
        "term",
        "ARS can refer to one of two rotation systems:\nArika Rotation System, a rotation system used in the Tetris: The Grand Master series, or\nAtari Rotation System, a rotation system that aligns pieces to the top left corner when rotating.",
    },
    {"ASC",
        "ascension",
        "term",
    ...
    {"SRS+",
        ...
        "term",
        "An extension to SRS that adds support for 180° wall kicks.",
    },
    {"TRS",
    ...
    {"Hold",
        "hold",
        "term",
        "\nA function that allows the player to store the current block in a reserve (if there are no blocks in Hold) or swap the current block with the block in the reserve. Normally, Holds can only be used once per block. Holds allow the player to adjust the order of the blocks and make it easier to build the desired setup.\nTechmino has a \"In-Place Hold\" function that would spawn the block in the Hold at the same location as the current block.\nThere are also arguments for not using Holds. It can be argued that not using Holds allows a player to concentrate more on the sequences at hand, which could reduce the player's workload. It could also reduce the variety of keys or buttons used during the game, which could increase the KPS. In fact, many world records on 40L are set without Holds. However, some people may also argue that Holds are useful for adjusting the upcoming sequence and reducing the difficulty in many circumstances (such as high gravity), allowing the player to achieve more complex setups without significantly increasing the workload, which could reduce the total number of key presses.",
    },
    {"Swap",
        "hold",
        "term",
        "Another possible form of Hold. Swaps allow the player to swap the current block with the next block. Normally, Swap can only be used once per block.",
    },
    {"Deepdrop",
        "shenjiang",
        "term",
    ...
    {"Donation",
        "donate",
        "term",
        "Converting a hole (usually originally intended for Tetries) into a T-spin by \"plugging\" it to achieve a future T-spin. Donation is an advanced skill for maintaining or increasing attacks. After the T-spin, the hole would open again to allow the continuation of Tetris or another donation. Plugging a hold not intended for Tetrises is sometimes colloquially referred to as \"donation.\".",
    },
    {"'Debt'",
        "qianzhai debt owe",
        "term",
    ...
    {"Center well",
        ...
        "term",
        "A special stacking method that leaves a well of a certain width in the center of the field. This allows the player to make combos without the risk of topping out as in side wells.",
    },
    {"Partial well",
    ...
    {"Side 2-wide",
        "s2w side2wide",
        "term",
        "Or S2W for short. Leaving a two-block-wide well on the side of the field, a common combo setup.\nS2Ws are easy to build and can create relatively long combos when used with Holds. However, it is less commonly used by advanced players due to its long setup time, susceptibility to incoming attacks during that time, and relatively low efficiency. This can result in fewer combos or top-outs.",
    },
    {"Side 3-wide",
        "s3w side2wide",
        "term",
    ...
    {"6–3 Stacking",
        ...
        "term",
        "A special stacking method where a six-block-wide stack is placed on the left side of the board and a three-block-wide stack is placed on the right side of the board, resulting in a one-block-wide well off-center.\nThis method could reduce the number of keypresses for a skilled player and is popular in sprint modes to increase speed. The reason for this may have to do with the fact that blocks spawn with a bias to the left. Notice that this method may increase keypresses for beginners.",
    },
    {"Freestyle",
        "ziyou",
        "term",
        "A term commonly used for TSD challenges. This term means completing a certain number of T-spin doubles in a challenge without using a fixed stacking setup. Compared to fixed stacking setups such as LST, freestyles are more difficult and more representative of a player's T-spin skills in real-world battles.",
    },
    {"Topping out",
    ...
    {"Auto-lock cut",
        "autolockcut mdcut",
        "term",
        "A feature in Techmino that prevents a block from being misdropped after a new block is spawned. Hard drops can be disabled for a short time after the previous block has been locked.\nOther games may have similar mechanisms, but they may work differently.",
    },
    {"SDF",
        "softdropfactor",
        "term",
        "Soft Drop Factor, a number describing the relative speed of soft drops in some games. In many modern Tetris games, soft drops are realized by speeding up the natural drop by a constant factor, the SDF. Guideline Tetris games usually have an SDF of 20, i.e. the soft drop speed is 20 times the natural drop speed.\n Techmino uses a constant soft drop speed (described by ARR) instead of the SDF to determine soft drop speeds.",
    },
    {"Shape & Names",
        "mino",
        "term",
    ...
    {"bagES generator",
        "bages easy start",
        "term",
        "Bag Easy-Start, a derivative of the bag generator used in Techmino. The first block in the first bag will not be S, Z, S5, Z5, F, E, W, X, N, or H, which can be difficult to place at the beginning.",
    },
    {"Reverb generator",
        "reverb",
        "term",
    ...
    {"MPH mode",
        "mph",
        "term",
        "A game mode. MPH is an acronym for Memoryless (the mode uses a simple random generator), Previewless (no next queue), Holdless (no hold). This mode challenges the player's reaction speed.",
    },
    {"Input delay",
        "input delay",
        "term",
