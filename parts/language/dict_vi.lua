...
Ví dụ: nếu muốn lọc các mục trong nhóm "Các game xếp gạch", hãy gõ vào thanh tìm kiếm cú pháp "nhom6". Hoặc để xem các hệ thống xoay gạch, gõ "nhom5c"
        ]]
    },
    {
        "=[NHÓM 2]=",
...
[…] (Tiếp tục từ trang 3 | Phần đầu tiên của mục này nằm ở trang 2)
        5H. Khả năng tấn công
            * APM, SPM, DPM, RPM, ADPM, APL
            * Tấn công & Phòng thủ
            * Combo, Spike
            * Debt
            * Passthrough
        5I. Hành động bất cẩn (Mis-): Misdrop, Mishold
        5J. Spin
            * (Mini)/(All-)/(T-)/(O-) spin
            * Fin, Neo, Iso
        5K: Kỹ thuật xóa hàng:
            * Single, Double, Triple, Techrash, Tetris
            * Perfect Clear, Half Perfect Clear
            * TSS, TSD, TST, MTSS, MSTSD
        5L. Các thuật ngữ khác: sub, 'Doing Research', Bone block
        ]]
    },
    {
        "Mục lục [5/8]", 
...
-----------------
    ...
    {"Website chính thức",
        "nhom3 homepage mainpage websites trang chủ ",
        "org",
    ...
    {"Dự án trên GitHub",
        "nhom3 githubrepository sourcecode src".."mã nguồn mở  dự án  kho lưu trữ",
        "org",
...
Dưới đây là một số quy tắc chung, nhưng chúng không phải là quy tắc bắt buộc
    ...
    {"Topping out",
        "nhom5a die death topout toppingout",
        "term",
        [[
...
Techmino không kiểm tra điều kiện Lock out và Top out.
    ...
    {"Vùng đệm",
        "nhom5a above super invisible disappear buffer zone",
        "term",
    ...
    {"Vùng biến mất",
        "nhom5a disappear gone cut die vanish zone",
        "term",
    ...
    {"H.dạng của Tetro.",
        "nhom5b shape structure form tetromino tetrimino hình dạng",
        "term",
    ...
    {"Màu của Tetromino",
        "nhom5b colour hue tint tetromino tetrimino màu",
        "term",
    ...
    {"Gạch & tên tg. ứng",
        "nhom 5b mino gạch & tên tương ứng",
        "term",
    ...
    {"Hướng gạch",
        "nhom5b direction 0r2l 02 20 rl lr",
        "term",
        [[
...
P/s: trong Zictionary (TetroDictionary), từ “bảng các vị trí 'đá' tường” viết tắt là “bảng 'đá' tường” (do lười gõ, mà cái này có thể thay đổi sau)
    ...
    {"ARS",
        "nhom5c arikrotationsystem atarirotationsystem",
        "term",
...
Khoảng cách euclide của độ lệch của cú đá được chọn không được lớn hơn √5; và nếu có độ lệch theo chiều ngang, thì hướng của cú đá đó không được là hướng ngược lại với hướng đã chọn (từ việc giữ phím)
    ...
    {"SRS+",
        "nhom5c srsplus superrotationsystemplus",
        "term",
        "Một biến thể của SRS, hỗ trợ bảng 'đá' tường khi xoay 180°.",
    },
    {"TRS",
        "nhom5c techminorotationsystem",
        "term",
    ...
    {"Kiểu xáo Túi 7",
        "nhom5e bag7bag randomgenerator",
        "term",
        [[
...
Cuối cùng là thêm gạch vào chuỗi NEXT và quay về bước đầu tiên.
    ...
    {"Kiểu xáo EZ-Start",
        "nhom5e bages easy start khởi đầu suôn sẻ",
        "term",
    ...
    {"Kiểu xáo C2",
        "nhom5e cultris2generator cultrisiigenerator c2generator",
        "term",
...
Trong Techmino, tốc độ rơi được mô tả là số khung hình cần thiết để gạch rơi xuống một đơn vị; ví dụ, 60 để chỉ gạch rơi mỗi ô một giây (mà game mặc định chạy ở 60FPS).
    ...
    {"Lockdown Delay",
        "nhom5f1 lockdelay lockdowndelay lockdowntimer",
        "term",
...
Line Clear Delay (Thời gian chờ xóa hàng): Thời gian để hiệu ứng xóa hàng thực hiện xong.
    ...
    {"ARE",
        "nhom5f1 spawn appearance delay",
        "term",
    ...
    {"DAS (đơn giản)",
        "nhom5f2 das arr delayedautoshift autorepeatrate",
        "term",
...
Mỗi game có cách tính LPM khác nhau. Ví dụ như, Tetris Online tính LPM bằng cách sử dụng PPS (nhìn mục ở bên dưới), trong đó 1 PPS = 24 LPM; do đó số hàng rác sẽ không được tính vào LPM và làm cho LPM khác với nghĩa đen của nó. Trong Techmino, giá trị LPM theo cách tính của Techmino gọi là "L'PM"
    ...
    {"PPS",
        "nhom5g1 piecespersecond speed",
        "term",
        "Pieces per second | Số gạch mỗi giây\n\tPhản ánh tốc độ chơi.",
    },
    {"BPM",
        "nhom5g1 blocksperminute piecesperminute speed",
        "term",
...
Phương pháp này lần đầu tiên được phát hiện ra bởi Cheez-fish - người đã đạt tốc độ nhấn lên tới 20 Hz.
    ...
    {"APL",
        "nhom5h attackperline efficiency",
        "term",
    ...
    {"Tấn công & Phg thủ",
        "nhom5h attacking defending phòng thủ",
        "term",
        [[
