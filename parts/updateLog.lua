return[=[
未来模式:
        任务树; 对称; 大爆炸; 拼方形; 限高无尽挖掘; 连击练习; 术语问答
        极简教程/考试; backfire对战; 强制Misdrop; 仅4lPC
未来小游戏:
        Tetro-1010(四/五连块, 2C2N, 若干回合改变重力方向)
        Tetra-link; 噗哟; 泡泡龙; 求合体; 坦克大战; 扫雷; 接水管
其他未来内容:
        可调场地宽度; 可调攻击系统; 组队战; 实时统计数据可视化; 教学关
        重做模式选择UI; 重做模组UI; 高级自定义序列; 加速下落; spike相关统计数据
        方块位移/旋转动画; 更好的手柄支持; 场地格子边缘线; 模式数据分析; C2连击
        Domain状态; 手势操作; 特殊控件(虚拟摇杆等); 交换(对应暂存)
        区分各种消除(隔断/架空/混合/彩色/穿墙)
        更复杂的攻击参数(数量/可抵消延时/不可抵消延时/反击/攻击/防御/洞数/连接/炸弹/厚度)
