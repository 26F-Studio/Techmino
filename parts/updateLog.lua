...
其他未来内容:
        可调场地宽度; 可调攻击系统; 组队战; 实时统计数据可视化; 教学关
        重做模式选择UI; 重做模组UI; 加速下落; spike相关统计数据; 录像继续
        更好的手柄支持; 场地格子边缘线; 模式数据分析; 高级自定义序列
        成就系统; Domain状态; 手势操作; C2连击; 特殊控件(虚拟摇杆等)
        更多消除方式(隔断/架空/混合/彩色/穿墙)
        更复杂的攻击参数(数量/倒计时/可抵消延时/不可抵消延时/反击/攻击/防御/洞数/连接/炸弹/厚度)
        方块位移/旋转动画; 更细节的DAS选项; 拓展主题系统
        工程编译到字节码; task-Z(新AI)
        等级系统; 收集向抽奖; 自适应UI; 多方块
...
0.16.2: 曲率引擎 Warp drive
        新增:
                ...
                新机制:swap(hold的另一种表现方式)
                新BGM:Jazz nihilism(用于节日主题,by Trebor)
                新BGM:Null(用于节日主题)
                新模式:更低和更高速度开始的经典模式(by not-a-normal-bot) #285
                新模式:大师-M
                新音效:单次消5/6行
