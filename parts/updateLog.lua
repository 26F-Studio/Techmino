...
"Patron"(time ordered, may not accurate)         Thanks!!!
		...
		<rmb10+>
				...
				彼方云城    pm250    nanaya    黑红蓝白    细胞    流光梦    George2003
未来计划:
		新模式:
				无尽PC挑战; 简单极简练习; 任务生存; 对称; 跑酷; 教学; 术语问答
				极简教程/考试; 连击练习; 自攻自受; 大爆炸; 音游模式; 拼方形
				OSD; 强制Misdrop; 涂色模式(模仿喷喷, designed by teatube)
		其他:
				小游戏:
						速算(篡改算符优先级,前缀后缀表达式)
						Tetro-1010(四/五连块, 2C2N, 若干回合改变重力方向)
						求合体; 坦克大战; 扫雷; 各种NS计算器小游戏移植
				物理hold; 多方块; 自由场地尺寸; XRS
				区分各种消除(隔断/架空/混合/彩色/穿墙)
				更复杂的垃圾行(数量/等待时间/抵消倍率/洞数/连接/炸弹)
				移动n格+硬降复合操作键; 更好的手柄支持; 手势操作
				DAS系统更细节的折返选项; 特殊控件(虚拟摇杆等)
				画图智能画笔; 自定义游戏按各种目标复制数据; 15p计步拓展
				主菜单和选关UI重做; 适应任意屏幕尺寸的UI
				可更换的全局主题系统; 成就系统; 3D背景
				可选虚拟按键颜色; 工程编译到字节码; task-Z(新AI)
				录像回放菜单; 跳帧开关; 超60帧; 热更新; 命令行
